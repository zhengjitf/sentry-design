import { Dsn, encodeUrlParams, getReportDialogEndpoint } from '../src/dsn';
...
describe('encodeUrlParams()', () => {
  test('returns empty array for empty object input', () => {
  ...
  test('returns multiple key/value pairs', () => {
    expect(encodeUrlParams({ foo: 'bar', pickle: 'rick', morty: '4 2' })).toEqual([
      'foo=bar',
      'pickle=rick',
      'morty=4%202',
    ]);
